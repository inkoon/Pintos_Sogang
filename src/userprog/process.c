...
/* Projrct 1 */
int parse_filename(char *src, char *dest[]){
	char *temp;
	char *save_ptr;
	int argc = 0;

	temp = strtok_r(src," ",&save_ptr);
	while(temp != NULL){
		dest[argc++] = temp;
...
void construct_esp(void **esp, int argc, char *argv[128]){
	char **arg_addr;
	int i,len,word_align=0;
...
{
	int rval = -1;
	struct thread *c = find_thread(child_tid);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Project 1 */
	args = (char*)malloc(sizeof(char) * (strlen(file_name) + 1));
	strlcpy(args,file_name,strlen(file_name)+1);
	argc = parse_filename(args,argv);
